        ...
        if ($headBlock = $this->getLayout()->getBlock('head')) {
            ...
            if ($this->helper('catalog/category')->canUseCanonicalTag()) {
                ...
                if($rewrite == 1) {
                    ...
                    $canonical_url = Mage::getModel('filterurls/catalog_layer_filter_item')->getSpeakingFilterUrl(FALSE, TRUE);                        
                    
                    if ($clink = Mage::registry('canonical_link')) {
                        $canonical_url = Mage::getUrl($clink);
                    }                    
                    $headBlock->addLinkRel('canonical',  $canonical_url);                    
                }
                else{
