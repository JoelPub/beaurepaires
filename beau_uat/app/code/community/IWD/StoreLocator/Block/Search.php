...
class IWD_StoreLocator_Block_Search extends Mage_Core_Block_Template{
	...
	public function getCountries(){
		$result = array();
		$collection = Mage::getModel('storelocator/stores')->getCollection()->addFieldToFilter('is_active', array('eq'=>'1'));
	...
	public function getJsonConfig($campaign=false){
	    ...
	    $config->setData('zoomData', $this->getZoom());
            ...
            if($campaign)
            $config->setData('urlSearch', Mage::getUrl('storelocator/json/search_10_kms', array('_secure'=>$_secure)));    
            else                
	    $config->setData('urlSearch', Mage::getUrl('storelocator/json/search', array('_secure'=>$_secure)));
