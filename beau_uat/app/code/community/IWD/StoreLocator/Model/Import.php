...
class IWD_StoreLocator_Model_Import extends Mage_ImportExport_Model_Abstract {
    ...
    private function createRecord($row) {
        $head = $this->_currentHead;
        $model = Mage::getModel('storelocator/stores');        
        $email = $row[$head['email']['index']];        
        
        if(trim($email)!='') {
            
            $collection = $model->getCollection()->addFieldToFilter('email', array('eq'=>$email));
        ...
        foreach ($head as $field => $data) {
            ...
            //get store category code and convert them to id
            if($field == 'type_id'){
                $codeValue = strtoupper($value);
                $codeValue = trim($value);
                $value = Mage::helper('storecategory')->getIdFromCode($codeValue);                                
            }

            // Yes / No fields -> 1 or 0. Also fix 'true' and '1'
            if (in_array($field, 
                    array(
                        'comm_tyres', 'cons_tyres', 'brakes_service', 'balancing', 
                        'wheel_alignments', 'batteries', 'nitrogen', 
                        'wheelchair_access', 'drop_off', 'mobile_fleet', 
                        'waiting_area', 'guest_wifi', 'guest_tablet', 'coffee_tea','exclude_from_cart'))) {
                
                $value = strtolower($value);
                $value = $value[0];
                if ($value == 'y' || $value == 't' || $value == '1' || $value == 'Yes' || $value == 'yes') {
                    $value = 1;
                }
                else {
                    $value = 0;
            ...
            // $value = trim($value); // this is duplicated
            if ($field == 'stores') {
                $value = explode(',', $value);
            }
