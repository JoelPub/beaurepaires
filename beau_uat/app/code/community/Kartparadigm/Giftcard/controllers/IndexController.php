...
class Kartparadigm_Giftcard_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function indexAction() {
		...
		$group_id = $collection->getGroupId();
        $customerGroupId = Mage::getSingleton('customer/session')->getCustomerGroupId();
		
        if (/*$group_id <= 0 ||*/ $group_id == $customerGroupId || ($canothersbuy && $customerGroupId <= 0)) {
            ...
            if (Mage::getStoreConfig('giftcard/giftcard/suborgrand_select', Mage::app()->getStore()->getStoreId()))
                ...
                $total = $quote->getSubtotal(); //apply gc to subtotal
            ...
            /* else if (!$canothersbuy && ($customermail!=$collection['receiver_mail'])) {
              ...
              $this->_redirect('checkout/cart/');
                ...
                if (isset($giftbal)) {
                    ...
                    if (isset($data['redeem_amt']) && ($data['redeem_amt'] > 0) && ($data['redeem_amt'] <= $giftbal))
                        $amt = $data['redeem_amt'];
                    else
                        $amt = $giftbal;
                    ...
                    Mage::getSingleton('core/session')
                            ->addError("Gift voucher '" . $data['gc_code'] . "'  is not valid");
                }
                $this->_redirect('checkout/cart/');
            }
        } else {
