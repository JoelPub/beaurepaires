...
class Plumrocket_SocialLogin_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loginAction()
    {
        ...
        if($call = $this->_getHelper()->apiCall()) {
            if(isset($call['type']) && $call['type'] == $type && !empty($call['action'])) {
                ...
                if(count($_target) === 3) {
                	...
                	Mage::log("There is problems in social media callback", null, "social.debug.log");
                    $this->_windowClose();
                    return;
                }
            }
        ...
        if ($session->isLoggedIn() && !$callTarget) {
        	Mage::log("Session is logged in however call call target is none", null, "social.debug.log");
            return $this->_windowClose();
            // $this->_redirect('.');
        }
        ...
        $className = 'Plumrocket_SocialLogin_Model_'. ucfirst($type);
        if(!$type || !class_exists($className)) {
        	Mage::log("Class is not found", null, "social.debug.log");
            return $this->_windowClose();
        ...
        if(!$model->loadUserData($response)) {
        	Mage::log("Can't find user data", null, "social.debug.log");
            return $this->_windowClose();
            // $this->_redirect('customer/account/login');
        }
        ...
        if($callTarget) {
            ...
            Mage::log("forward to calltarget", null, "social.debug.log");
        ...
        if($this->getRequest()->isXmlHttpRequest()) {
        	Mage::log("XML succeed", null, "social.debug.log");
            ...
            Mage::log("None xml succeed", null, "social.debug.log");
