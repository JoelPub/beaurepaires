<?php
class ApdInteract_Vir_Adminhtml_ReportcommercialController extends Mage_Adminhtml_Controller_Report_Abstract
{

    const CSV_FILE_NAME = "vir_report_commercial";

    public function indexAction(){

        $this->_title($this->__('Reports'))->_title($this->__('VIR'))->_title($this->__('VIR Commercial Vehicle'));
        $this->_initAction();
    ...
    /**
     * Export VIR Commercial Report
     */
    public function exportReportCommercialCsvAction(){
        ...
        if($from_date != '' && $to_date != ''){
            ...
            $vir_commercial->addFieldToFilter('created_at',array($filter_date));
        }elseif($from_date != '' && $to_date == ''){
            $from_date = date('Y-m-d',strtotime($from_date));
            $vir_commercial->addFieldToFilter('created_at',array('gteq' => $from_date));
        }elseif($from_date == '' && $to_date != ''){
            $to_date = date('Y-m-d',strtotime($to_date));
            $vir_commercial->addFieldToFilter('created_at',array('lteq' => $to_date));
        }

        //Filter Status
        if($status != '' && $status != 'empty'){
            $vir_commercial->addFieldToFilter('status',array('eq'=>$status));
        }elseif($status == 'empty'){
            // show all
        }else{
            $vir_commercial->addFieldToFilter('parent_id',array('eq'=>0));;
        }

        $this->saveToCSV($vir_commercial);
