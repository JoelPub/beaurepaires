...
class ApdInteract_ImportExport_Model_Export_Entity_Productinventory extends Mage_ImportExport_Model_Export_Entity_Abstract
{
        ...
        while (true) {
            ...
            if ($offsetProducts == 1) {
                ...
                $headerCols = array_merge(
                    array(
                        self::COL_SKU,self::COL_CATEGORY
                    ),

                    array(
                        'name','qty','is_in_stock','manage_stock'
                    )
                );
    ...
    {
        $attributeCodes = array('store_id','created_at','sku','category_main','status','name');

        return Mage::getResourceModel('catalog/product_attribute_collection')->setCodeFilter($attributeCodes);
    }
