<?php
/**
 * Copy all CMS pages and CMS blocks from Beaurepaires default website to Goodyear NZ website
 */
...
if ($sourStore->getId() && $destStore->getId()) {
    ...
    foreach ($pageCollection as $cmsPage) {
        ...
        if (preg_match('/(^| )Australia( |$)/im', $newCmsPage->getTitle())) {
            $newCmsPage->setTitle(preg_replace('/(^| )Australia( |$)/im', '$1New Zealand$2', $newCmsPage->getTitle()));
        }
    ...
    foreach ($blockCollection as $cmsBlock) { 
        ...
        if (preg_match('/(^| )Australia( |$)/im', $newCmsBlock->getTitle())) {
            $newCmsPage->setTitle(preg_replace('/(^| )Australia( |$)/im', '$1New Zealand$2', $newCmsBlock->getTitle()));
        }
