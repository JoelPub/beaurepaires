...
class GoMage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function ajaxAction()
    {
        ...
        try {
            switch ($action) :
                ...
                case ('cartupdate'):
                    $qtyChange = $this->getRequest()->getParam('qty_change');
                    $itemId = $this->getRequest()->getParam('id');
                    if ($itemId && $itemId>0 && $qtyChange && $qtyChange > 0) {
                ...
                case ('cartremove'):
                    ...
                    if ($itemId = $this->getRequest()->getParam('id')) {
                        ...
                        $items = Mage::getModel('checkout/cart')->getQuote()->getAllItems();//$cartHelper->getCart()->getItems();
                        ...
                        //product 3113
                        if($productId > 0 && $this->getRequest()->getParam('allow_gift_options')) {
                            $this->removeSessionGiftItem($productId);
                        }
                ...
                case ('get_methods'):
                    ...
                    if ($billing_address_data = $this->getRequest()->getPost('billing')) {
                        if (isset($billing_address_data['use_for_shipping']) && $billing_address_data['use_for_shipping'] == 'collect_advintage') {
                            $this
                                ->getOnepage()
                                ->getQuote()
                                ->setCollectAdvintage(true)
                                ->save();
                        } else {
                            $this
                                ->getOnepage()
                                ->getQuote()
                                ->setCollectAdvintage(false)
                                ->save();
                        }
                ...
                case ('save_payment_methods'):
                    ...
                    if($billing_address_data['year'] > date("Y", Mage::getModel('core/date')->timestamp(time())) - 18) {
                        $calculator->result->error     = true;
                        $calculator->result->message[] = $this->__('Incorrect date of birdhday, you must be over 18.');
                    } elseif($billing_address_data['year'] == date("Y", Mage::getModel('core/date')->timestamp(time())) - 18) {
                        if($billing_address_data['month'] > date("m", Mage::getModel('core/date')->timestamp(time()))) {
                            $calculator->result->error     = true;
                            $calculator->result->message[] = $this->__('Incorrect date of birdhday, you must be over 18.');
                        } elseif($billing_address_data['month'] == date("m", Mage::getModel('core/date')->timestamp(time()))) {
                            if($billing_address_data['day'] > date("d", Mage::getModel('core/date')->timestamp(time()))) {
                                $calculator->result->error     = true;
                                $calculator->result->message[] = $this->__('Incorrect date of birdhday, you must be over 18.');
                            }
                        }
                ...
                case ('gift_option'):
                    ...
                    //GiftOption
                    $giftoption = Mage::getModel('gomage_checkout/giftoption');
                    $giftoption->updateSession();
                    $giftoption->captureBeforePaymentInfo();
                    ...
                    if (!$this->getOnepage()->getQuote()->isVirtual()) {
                        ...
                        if($customgiftoption && isset($customgiftoption['gift-box']) && $customgiftoption['gift-box']) {
                            $calculator->result->gift_message = Mage::helper('gomage_checkout/giftMessage')->getInline('onepage_checkout', $this->getOnepage()->getQuote());
                        }
                    } else {
                        $calculator->generateResult(array('methods', 'review'));
                    ...
                    Mage::getSingleton('core/session')->setPayment($this->getRequest()->getParam('payment'));
                    Mage::getSingleton('core/session')->setSaveLeaveWithoutSignature($this->getRequest()->getParam('leave_without_signature'));
                    break;

                case 'review':
                    $result_blocks = array('review');
                    $calculator->generateResult($result_blocks);
                    break;
            endswitch;
    ...
    public function saveAction()
    {
        ...
        if ($this->getRequest()->isPost() && $post = $this->getRequest()->getPost() && !isset($result['redirect'])) {
            ...
            try {
                ...
                $giftoption->updateSession();
                $giftoption->captureBeforePaymentInfo();*/
                if(Mage::app()->getRequest()->getParam('allow_gift_options')) {
                    $customgiftoption = Mage::app()->getRequest()->getParam('customgiftmessage');
                    $gift_comments = isset($customgiftoption['gift_comments']) ? $customgiftoption['gift_comments'] : false;
                    Mage::getSingleton('checkout/session')->getQuote()->setGiftComments($gift_comments)->save();
                }
                ...
                if($billing_address_data['year'] > date("Y", Mage::getModel('core/date')->timestamp(time())) - 18) {
                    ...
                    $result['message'][] = $this->__('Incorrect date of birdhday, you must be over 18.');
                } elseif($billing_address_data['year'] == date("Y", Mage::getModel('core/date')->timestamp(time())) - 18) {
                    if($billing_address_data['month'] > date("m", Mage::getModel('core/date')->timestamp(time()))) {
                        $result['error']      = true;
                        $result['success']   = false;
                ...
                if (!$this->getOnepage()->getQuote()->isVirtual()) {
                    ...
                    if ((!isset($billing_address_data['use_for_shipping']) || !intval($billing_address_data['use_for_shipping']) && (isset($billing_address_data['use_for_shipping']) && $billing_address_data['use_for_shipping'] != 'collect_advintage'))) {
                ...
                if(isset($post['leave_without_signature'])) {
                    Mage::getSingleton('core/session')->setLeaveWithoutSignature($post['leave_without_signature']);
                }
     ...
     * @param $p3113
     */
    public function removeGiftItem() {
        ...
        $productsGB = Mage::getModel('catalog/category')->load($category_id)
        ...
        $arrIn = $arrOut = array();
        foreach ($items as $item) {
            if($p3113 && $p3113->getId() == $item->getProductId()) {
                $arrOut[] = $item->getItemId();
            } else {
                foreach ($productsGB as $p) {
                    if($p->getId() == $item->getProductId()) {
                        $arrOut[] = $item->getItemId();
                    }
                }
            }
            $arrIn[] = $item->getItemId();
        }
//        if (count($arrIn) > 0 && count($arrIn) == count($arrOut)) {
//            // Empty shopping cart
//            Mage::getSingleton('checkout/cart')->truncate();
//            Mage::getSingleton('core/session')->setStylishCard(false);
//            Mage::getSingleton('core/session')->setGiftBox(false);
//            Mage::getSingleton('core/session')->addError(__('Sorry. You could not checkout with only Stylish Gift Card and Gift Box product'));
//        }
        unset($p3113);
        unset($item);
     ...
     * @param $productId
     */
    public function removeSessionGiftItem($productId) {
        $p3113 = Mage::getModel('catalog/product')->loadByAttribute('sku', '3113');
        $category_id = (int) Mage::getStoreConfig('advintage_category/category_general/giftBoxesId');
        $productsGB = Mage::getModel('catalog/category')->load($category_id)
            ->getProductCollection()
        ...
        if($p3113 && $p3113->getId() == $productId) {
            //Stylish Gift Card
            Mage::getSingleton('core/session')->setData('stylish_card');
        } else {
            foreach ($productsGB as $p) {
                if($p->getId() == $productId) {
                    //Single Bubbly Gift Box
                    Mage::getSingleton('core/session')->setData('gift_box');
                    break;
                }
            }
        }
