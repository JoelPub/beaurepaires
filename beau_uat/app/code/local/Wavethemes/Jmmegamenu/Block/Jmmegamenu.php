...
class Wavethemes_Jmmegamenu_Block_Jmmegamenu extends Mage_Page_Block_Html_Topmenu {
    ...
    public function _toHtml() {
        ...
        $collections = Mage::getModel('jmmegamenu/jmmegamenu')
                ->getCollection()
                ->setOrder("parent", "ASC")
                ->setOrder("ordering", "ASC")
                ->addFieldToFilter("status", 1, "eq")
                ->addFieldToFilter("menugroup", $rows["id"]);
        ...
        $menuoutputend = str_replace(array('title-area', '<span>Menu</span>'), array('title-area inline-list', '<span></span>'), $menuoutput);
        $this->assign('menuoutput', $menuoutputend);
        ob_end_clean();
        return parent::_toHtml();
    }
