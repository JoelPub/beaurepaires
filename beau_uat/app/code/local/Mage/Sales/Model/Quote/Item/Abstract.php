    ...
    implements Mage_Catalog_Model_Product_Configuration_Item_Interface
{
    ...
    public function calcRowTotal()
    {
        ...
        // Added upsells directly to row total * quantity since custom option implementation is not using the default magento
		$options = Mage::helper('apdinteract_catalog')->getCustomOptions($this); 
		$upsells = 0;
		foreach($options as $costx)
		{ 
			$upsells += $costx['price'];
		}
		$upsellsTotal = $qty * $upsells;
		if ($product->getTypeId() == 'configurable'){
			// need to add the upsells total directly to configurable
			$this->setRowTotal($this->getStore()->roundPrice($total + $upsellsTotal));
			$this->setBaseRowTotal($this->getStore()->roundPrice($baseTotal + $upsellsTotal));
		}else{
			// for simple products custom options computation are working out of the box.
