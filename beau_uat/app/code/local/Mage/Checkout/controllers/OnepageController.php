...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
     ...
     */
    public function saveBillingbackAction()
    {
        if ($this->_expireAjax()) {
            return;
        }
...
$this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
     ...
     */
    public function saveShippingbackAction()
    {
        if ($this->_expireAjax()) {
            return;
        }
...
$this->getResponse()->setBody(Mage::helper('core')->jsonEncode($result));
    ...
    public function saveOrderAction()
    {
        ...
        try {
            ...
            $invalid_ip_error = 'Calendar add appointment returned invalid response: Not permitted from';
            if (strpos($exception_message, $invalid_ip_error) !== false) {
                Mage::log($e->getMessage(), null, 'calendar_blocked_ips.log');
            }
            
            if ($exception_message == 'Calendar add appointment returned invalid response: Appointment Details Invalid') {
                $result['error_messages'] = $this->__('Sorry, your chosen fitment time has been taken by another customer. Please select an alternative time, date or store.');
