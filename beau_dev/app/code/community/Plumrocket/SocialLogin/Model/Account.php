...
class Plumrocket_SocialLogin_Model_Account extends Mage_Core_Model_Abstract
{
    ...
    public function registrationCustomer()
    {
        ...
        try{            
            $customer->setData($this->getUserData())
                ->setConfirmation($this->_getRandomPassword())
                ->setPasswordConfirmation($this->_getRandomPassword())
                ->setData('is_active', 1)
                ->getGroupId();
    ...
    protected function _getRandomPassword()
    {
        ...
        $key .= $num[mt_rand(0,1)];
        Mage::log("KEY: ".$key, null, "social.debug.log");
        return $key;
    }
