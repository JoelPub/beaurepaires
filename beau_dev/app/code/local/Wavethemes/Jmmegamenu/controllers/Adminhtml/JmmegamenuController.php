...
class Wavethemes_Jmmegamenu_Adminhtml_JmmegamenuController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _initAction() {
        ...
        $this->loadLayout()->_setActiveMenu('jmmegamenu/items')->_addBreadcrumb(Mage::helper('adminhtml')->__('Menu Manager'), Mage::helper('adminhtml')->__('Menu Manager'));
    ...
    public function editgroupAction() {
        ...
        if ($groupModel->getId() || $Id == 0) {
            ...
            //$this->_addContent($this->getLayout()->createBlock('auguria_sliders/adminhtml_sliders_edit_tab_content')->toHtml());
    ...
    public function savegroupAction() {
        if ($this->getRequest()->getPost()) {
            try {
                ...
                $storegroups = $helper->getStoreGroups($oldstoreid);
                $groupstores = $helper->getGroupStores($oldstoreid);
               ...
               $new=false;
                if (count($storecollection)<=0) {
                    ...
                    $storegroupModel->setstore_id($postData['storeid'])
                            ...
                            ->save();
                } else { //update menugroup if entry exists
                    $store_data = $storecollection->getFirstItem();
                    $id = $store_data->getId();
                    $group = $storegroupModel->load($id);
                    $group->setmenugroupid($groupModel->getId())
                            ->save();
                                    
                    
                }
                
                // delete if old store id doesnt matched
                if ($oldstoreid !== $postData['storeid'] && !$new) {

                    $storegrouptable = $resource->getTableName('jmmegamenu_store_menugroup');
                    $query = 'Delete '
                            . ' FROM ' . $storegrouptable
                            . ' WHERE menugroupid = "' . $this->getRequest()->getParam('id') . '" OR store_id = "' . $this->getRequest()->getParam('storeid') . '"';
                    ...
                    $storegroupModel
                            ->setstore_id($this->getRequest()->getParam('storeid'))
                            ->setmenugroupid($this->getRequest()->getParam('id'))
                            ->save();
                }
