...
class Wavethemes_Jmmegamenu_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function fixLink($itemUrl) {
        $itemUrl = str_replace(Mage::getUrl(), '', $itemUrl);        
        ...
        if (substr($itemUrl, -1) == '/') {
            $itemUrl = substr($itemUrl, 0, -1);
        }

        return $itemUrl;
    }

    public function parse_url($itemUrl){

        $itemUrl = str_replace(Mage::getUrl(), '', $itemUrl);
        $itemUrl = str_replace('index.php', '', $itemUrl);
 
        if (isset($itemUrl[0]) && $itemUrl[0] == '/') {
            $itemUrl = substr($itemUrl, 1);
        }
    ...
    public function getStoreGroups($oldstoreid){
        
        $resource = Mage::getSingleton('core/resource');
        $read = $resource->getConnection('core_read');        
        
        $query = 'SELECT store_id,id'
    ...
    public function getGroupStores($oldstoreid) {
    ...
    public function getLastGroupStores() {
