...
class ApdInteract_Vir_Block_Printconsumer extends Mage_Core_Block_Template {
    ...
    public function generatePdf() {
        ...
        $hc = Mage::getModel('apdinteract_vir/healthcheck')->getCollection()
                ->addFieldToFilter('vir_id', $vir->getParentId())
                ->getFirstItem();

        $workshopSignature = $this->_getJsignatureClass()->setBase30($vir->getWorkshopsignatureimage())->getPng();
        $customerSignature = $this->_getJsignatureClass()->setBase30($vir->getCustapprsignatureimage())->getPng();

        $layout = Mage::app()->getLayout();
        $page1 = $layout->createBlock('core/template')->setTemplate('apdinteract/vir/pdf/vir/order/page1.phtml')->assign('hc', $hc);
        $page2 = $layout->createBlock('core/template')->setTemplate('apdinteract/vir/pdf/vir/order/page2.phtml')->assign(array('vir' => $vir, 'workshopSignature' => $workshopSignature, 'customerSignature' => $customerSignature));
        $page3 = $layout->createBlock('core/template')->setTemplate('apdinteract/vir/pdf/vir/order/page3.phtml')->assign('vir', $vir);

        $blockRoot = $layout->createBlock('core/template')
            ->setTemplate('apdinteract/vir/pdf/vir/order.phtml')
            ->setChild('10.point', $page1)
            ->setChild('vir.1', $page2)
            ->setChild('vir.2', $page3);
        ...
        Mage::getSingleton('apdinteract_pdf/pdf')->setTitle($pdfTitle)->sendToPdf($blockRoot->toHtml(), array("Attachment" => false));
    }
