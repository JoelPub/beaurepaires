<?php
class ApdInteract_SalesRule_Model_Validator extends Mage_SalesRule_Model_Validator
{
		...
		foreach ($this->_getRules() as $rule) {
			...
			switch ($rule->getSimpleAction()) {
				...
				case Mage_SalesRule_Model_Rule::BUY_X_GET_Y_ACTION:
					...
					// since we are adding custom extras from the product price, we need to manually deduct it when shopping cart price rule is applied
					if ($qty <= $x){
						$lessTheExtrasFromTheItemPrice = 0;
					}else {
						$lessTheExtrasFromTheItemPrice = $extras / $qty;
					}
					...
					$discountAmount    = $discountQty * $itemPrice - $lessTheExtrasFromTheItemPrice;
					$baseDiscountAmount = $discountQty * $baseItemPrice - $lessTheExtrasFromTheItemPrice;
					//get discount for original price
					$originalDiscountAmount    = $discountQty * $itemOriginalPrice - $lessTheExtrasFromTheItemPrice;
					$baseOriginalDiscountAmount = $discountQty * $baseItemOriginalPrice - $lessTheExtrasFromTheItemPrice;
					break;
					case ApdInteract_SalesRule_Model_Rule::BUNDLED_PRICE_ACTION:
