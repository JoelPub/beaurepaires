...
function validateQtyLimit(cdp) {
	var $qtyInput = cdp? jQuery('input.qty[data-cdp="'+ cdp +'"]') : jQuery('input.qty'),
			$qtyErrorTyres = jQuery('.qty-error[data-cdp="tyres"]'),
			$qtyErrorWheels = jQuery('.qty-error[data-cdp="wheels"]'),
			qtyTyresTotal = 0,
			qtyWheelsTotal = 0,
			qtyLimit = 5;

	$qtyInput.each(function(){
		var cdpType = jQuery(this).attr('data-cdp'), 
				value = 0;
		
		if (cdp) {
			value = parseInt(jQuery(this).val()); // get the real value
		} else {
			value = parseInt(jQuery(this).attr('value')); // get the real value
		}
		...
		if (cdpType === 'wheels') {
			qtyWheelsTotal += value;
		}
	});

	if (cdp) {
		jQuery('.qty-error[data-cdp="'+ cdp +'"]').addClass('hide');
	} else {
		$qtyErrorTyres.addClass('hide');
		$qtyErrorWheels.addClass('hide');
	}
	

	if (qtyTyresTotal > qtyLimit ||  qtyWheelsTotal > qtyLimit) {		
		if (qtyTyresTotal > qtyLimit) {
			$qtyErrorTyres.removeClass('hide');
		}

		if (qtyWheelsTotal > qtyLimit) {
			$qtyErrorWheels.removeClass('hide');
		}
...
function gocheckout() {
	...
	if ( storeLocVal == '' || dateVal == '' || availableTimesVal == '' || availableTimesVal == null) {
		...
		// validateQtyLimit(false); revert as Phil's advice
	} else {
		// if (!validateQtyLimit(false)) { // revert as Phil's advice
		...
		cartApp = {
			...
			'updateProduct': function( $link ) {
				var productId = $link.data('product_id'),
					...
					productCDP = $productElem.attr('data-cdp'),
					requestUrl = $link.data('update-url')+productValue;

				if (parseInt(productValue)) { //  && !validateQtyLimit(productCDP)
