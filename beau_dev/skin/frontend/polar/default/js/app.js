    ...
    $(function () {
        ...
        var beauComponents = {
                        ...
                        quickFinderElem.find('select').change(function (e) {
                            ...
                            switch (selectElem.attr('name')) {
                                ...
                                case 'year-tyres':
                                    ...
                                    $this._updateForm(
                                            {
                                                'vehiclemakeids': selectElem.val(), // before: 'vehiclemakeids': $this.selects['make-tyres'], 
                                                'year': $this.selects['year-tyres']},
                                                '/vehicles/models',
                                                'select.model-tyres',
                                                'select.model-tyres,select.series-tyres,.vehicle-type :submit'
                                            );
            ...
            'productPricing': {
                ...
                'setConst': function ($cart) {
                    ...
                    this.const = {
                        ...
                        '$productBadgeBlock': $('.product-badge-block'),
                        // config
                        'showPriceSuffix': '/tyre'
                    }
                },
                'handleScroll': function() {
                    if (!beauComponents.productPricing.const.$productBtns.length){return}
                ...
                'updateSizes': function ($field) {
                    ...
                    if ($field.is('select')) {
                        if ($field.find('option:selected').length) { // when there's an option selected, sometimes its empty
                            ...
                            if ($field.find('option:selected').data('badge')){
                                selectedDataBadge = $field.find('option:selected').data('badge');
                            }
                        }
                    ...
                    console.log('a: ', selectedDataBadge)
                    console.log('b: ', jQuery.type(selectedDataBadge))

                    // For Updating Badge
                    if (selectedDataBadge !== null) {
                        if (selectedDataBadge.length !== 0) {
                            var badgeValue = selectedDataBadge.value,
                                badgeLabel = selectedDataBadge.label,
                                tagClass = $this.const.$productBadgeBlock.attr('class').split(' ').pop();

                            $this.const.$productBadgeBlock.css('display', 'inline-block');
                            $this.const.$productBadgeBlock.removeClass(tagClass);
                            $this.const.$productBadgeBlock.addClass(badgeValue);
                            $this.const.$productBadgeBlock.text(badgeLabel);
                        } else {
                            $this.const.$productBadgeBlock.css('display', 'none');
                        }
                    }
            ...
            'dataBinding': function() {
                ...
                if (!$("[data-model]").length) return
                
                $("*[data-model]").on('change keyup', function (e) {  
                    var to_bind = $(this).attr('data-model'),
                        value = $(this).val()

                    if($(this).is("input")) {
                        // $("*[data-model='"+to_bind+"']").html(value);
                        $("*[data-model='"+to_bind+"']").val(value);
                    }

                    if($(this).is("select")) {
                        $("*[data-model='"+to_bind+"']").find('option[value="'+ value +'"]').prop('selected', true) 

                        if ($(this).hasClass('vehiclemodel-copy')) {
                            $('select.vehiclemodel-original').find('option[value="'+ value +'"]').prop('selected', true).trigger('change');
                        }
                    }
                })

                
                if ($('#custname_autocomplete_choices').length) {
                    $('#custname_autocomplete_choices').on('click', function(){
                        var value = $('#custname').val()

                        $("*[data-model='name']").val(value);
                    });
                }
                // $("div[data-model]").bind("DOMSubtreeModified",function(){
                //     var to_bind = $(this).attr('data-model');
                //     $("*[data-model='"+to_bind+"']").html($(this).html());
                //     $("*[data-model='"+to_bind+"']").val($(this).html());
                // });                 
            },
        'customerMobility': function(){
            ...
            'pdpReadMore': function () {
                var showLess = $('.show-for-small-only.show-less, .show-beau.show-less ');
                if(showLess.length){
                    showLess.find('.expanded').hide();
                    showLess.find('#readMore').on('click',function(e){
                        e.preventDefault();
                        showLess.find('.expanded').show();
                        showLess.find('.collapsed').hide();
                    });

                    showLess.find('#readLess').on('click',function(e){
                        e.preventDefault();
                        showLess.find('.collapsed').show();
                        showLess.find('.expanded').hide();
                    });
                }
            },
            'init': function () {
                ...
                beauComponents.pdpReadMore();
                beauComponents.dataBinding();
            }
        };
