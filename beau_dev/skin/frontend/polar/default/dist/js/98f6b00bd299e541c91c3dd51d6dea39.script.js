...
(function ($) {
    ...
    $(function () {
        ...
        'customerMobility': function(){
            ...
            'updateTagPDP': function(){
                if(!$('.product-options').length){return}
                var $selectedSize = $('.product-options').find('.select-size');
                changeBadge($selectedSize)
                $selectedSize.on('change', function(){
                    var $vm = $(this);
                    changeBadge($vm);
                });
                function changeBadge($sz) {
                    var $selectedOption = $sz.find('option:selected'),
                        $selectedDataBadge = $sz.find('option:selected').data('badge'),
                        $productBadgeBlock = $('.product-badge-block');
                    if($selectedOption.data('badge').length !== 0){
                        var $badgeValue = $selectedOption.data('badge').value,
                            $badgeLabel = $selectedOption.data('badge').label,
                            $tagClass = $productBadgeBlock.attr('class').split(' ').pop();
                        $productBadgeBlock.css('display', 'inline-block');
                        $productBadgeBlock.removeClass($tagClass);
                        $productBadgeBlock.addClass($badgeValue);
                        $productBadgeBlock.text($badgeLabel);
                    } else {
                        $productBadgeBlock.css('display', 'none');
                    }
                }
            },
            'init': function () {
